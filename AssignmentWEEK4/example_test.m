...
%% Plot LIDAR data
lidar_global(:,1) = (ranges(:,1).*cos(scanAngles + pose2(3,1)) + pose2(1,1))*param.resol + param.origin(1);
lidar_global(:,2) = (ranges(:,1).*sin(scanAngles + pose2(3,1)) + pose2(2,1))*param.resol + param.origin(2);
...
plot(pose2(1,:)*param.resol+param.origin(1), ...
    pose2(2,:)*param.resol+param.origin(2), 'b.-');